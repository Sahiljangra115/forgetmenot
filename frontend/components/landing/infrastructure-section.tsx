...
const regions = [
  { name: "Home care", nodes: 1, status: "core scenario" },
  { name: "Assisted living", nodes: 25, status: "pilot ready" },
  { name: "Memory clinics", nodes: 10, status: "pilot ready" },
  { name: "Hospital wards", nodes: 40, status: "pilot ready" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                ForgetMeNot runs in the browser and a lightweight backend beside it.
                Face recognition never leaves the device it happens on.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">128</span>
                <span className="text-2xl text-muted-foreground">numbers per face</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                That is the whole face: a 128-value embedding computed locally, the only thing that ever crosses the wire.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">2 FPS</span>
              <span className="block text-sm text-muted-foreground mt-2">Live recall stream</span>
            </div>
            ...
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">Face images stored, ever</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} profiles</span>
            </div>
          ))}
        </div>
      </div>
